...
def lambda_handler(event, context):
        ...
        # ARGUMENTS FOR THE HEATMAP 1 GO HERE
        ...
        # Analyte values
        i = np.argmin(abs(muCube[2][0] - muMat[2][2]))
        j = np.argmin(abs(muCube[3][0] - muMat[3][2]))

        sign = np.sign(ValCube[3][0][j])
        ...
        def update_species(species, ValCube, muCube, KaListCube, i, j):
            # Update only the required fields
            if 2 in species:
                species[2]['valence'] = [ValCube[2][0][i]]
                species[2]['mobility'] = [muCube[2][0][i]]
                species[2]['pKa'] = [-np.log10(KaListCube[2][0][i])]
            
            if 3 in species:
                species[3]['valence'] = [ValCube[3][0][j]]
                species[3]['mobility'] = [muCube[3][0][j]]
                species[3]['pKa'] = [-np.log10(KaListCube[3][0][j])]
