...
def find_CI_concentration(input_file, target_pH, LE_c):
    ...
    sol = minimize(
        LE_pH_to_target_pH_distance, LE_c, args=(input_file, target_pH, LE_c), tol=1e-6
    )
    return sol.x


def compute(LE_pH, LE_c, input_file, req_output="ATE_pH"):
    ...
    # Return output & check ITP conditions
    if req_output == "ATE_pH":
        output = pH[0, 2]
    elif req_output == "LE_pH":
        output = pH[0, 0]
    elif req_output == "sample_pH":
        output = pH[0, 1]
    elif req_output == "TE_pH":
        output = pH[0, 3]
    elif req_output == "sample_c_sample":
        output = cMat[2, 1]
    elif req_output == "sample_mob_sample":
        output = muMat[2, 1]
    elif req_output == "sample_cond":
        output = Sigma[0, 1]
    else:
        raise ValueError
...
species = speciesTest = { 0: {'Name': 'HCl', 'valence': [-1], 'mobility': [-79.1e-9], 
...
sol = compute(pH, LE_C, species, req_out)
